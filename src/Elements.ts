...
export function sugarSolution(rot: number): Operator {
  return Operator.outer([idDir, ops.rotationMatrix(rot * TAU, dimPol)])
}
...
export function mirror(rotState: number): Operator {
  ...
  return Operator.outer([Operator.fromSparseCoordNames(reflections[rotState], [dimDir]), ops.reflectPhaseFromDenser()])
}

export function beamSplitter(rotState: number): Operator {
  const reflections: [string, string, Complex][][] = [
    [
      // 0: -
      ["v", "^", Cx(1)],
  ...
  const passageDir = [
    Operator.fromSparseCoordNames([["^", "^", Cx(1)], ["v", "v", Cx(1)]], [dimDir]),
    idDir,
    Operator.fromSparseCoordNames([[">", ">", Cx(1)], ["<", "<", Cx(1)]], [dimDir]),
    idDir,
  ]
